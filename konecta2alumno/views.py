...
from konecta2app.models import EjerciciosAll,Permisos, MateriasEjercicios, EjerciciosPendientes, Corregir, Dificultad, Notificacion, CursosEjercicios, Tokenregister, Cursos, Profesor, Alumno, Invitado, Ejercicios
...
def ejercicios_pendientes(request):
    ...
    try:
        ...
        if comprobar_usuario.count() > 0:
            ...
            comprobar_ejercicios= EjerciciosAll.objects.filter(idalumno=alumno)
                ...
                for ejercicios in comprobar_ejercicios:
                    ...
                    if obtener_ejercicio.count() > 0:
                        ...
                        response_data['ejercicios'].append({'fecha': str(ejercicios.fecha_envio),
                                                            'idcorregir':ejercicios.idejerciciosall,
                                                            'idprofesor': ejercicios.idprofesor.idusuario.id,
                                                            'titulo': obtener_ejercicio.titulo,
                                                            'consejo': obtener_ejercicio.consejo,
                                                            'descripcion': obtener_ejercicio.descripcion,
                                                            'imagen': obtener_ejercicio.imagen,
                                                            'dificultad': obtener_ejercicio.dificultad.nombre,
                                                            'idejercicio': obtener_ejercicio.idejercicio,
                                                            'tipo': obtener_ejercicio.tipo.idtipo,
                                                            'interfaz': obtener_ejercicio.interfaz,
                                                            'calculadora': obtener_ejercicio.calculadora,
                                                            'materia': obtener_ejercicio.materia.nombre,
                                                            'idmateria': obtener_ejercicio.materia.idmateria})
            else:
